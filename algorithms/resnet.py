...
def _bn_relu(input_):
    ...
    """
    norm = BatchNormalization(axis=CHANNEL_AXIS)(input_)
...
def _conv_bn_relu(**conv_params):
    ...
    def f(input_):
        conv = Conv2D(filters=filters, kernel_size=kernel_size,
                      ...
                      kernel_regularizer=kernel_regularizer)(input_)
...
def _shortcut(input_, residual):
    ...
    if stride_width > 1 or stride_height > 1 or not equal_channels:
        shortcut = Conv2D(filters=residual_shape[CHANNEL_AXIS],
                          ...
                          kernel_regularizer=l2(0.0001))(input_)
    ...
    def f(input_):
        for i in range(repetitions):
            ...
            if i == 0 and not is_first_layer:
                init_strides = (2, 2)
            input_ = block_function(filters=filters, init_strides=init_strides,
                                   is_first_block_of_first_layer=(is_first_layer and i == 0))(input_)
        return input_
    ...
    def f(input_):
        ...
        if is_first_block_of_first_layer:
            ...
            conv1 = _bn_relu_conv(filters=filters, kernel_size=(3, 3),
                                  strides=init_strides)(input_)

        residual = _bn_relu_conv(filters=filters, kernel_size=(3, 3))(conv1)
        return _shortcut(input_, residual)
...
def _bn_relu_for_dense(input_):
    norm = BatchNormalization(axis=1)(input_)
    return Activation('relu')(norm)


def _top_network(input_):
    raw_result = _bn_relu_for_dense(input_)
...
class ResnetBuilder(object):
    ...
    def build(input_shape, num_outputs, block_fn, repetitions, is_classification):
        ...
        conv1 = _conv_bn_relu(filters=64, kernel_size=(7, 7), strides=(2, 2))(input_)
    ...
    def build_top_network(edge_model):
        ...
        input_ = Input(shape=(2 * NUM_EMBEDDING,))
        output = edge_model.layers[-number_of_top_layers](input_)  # _top_network(input)
    ...
    def build_siamese_resnet_18(input_shape, num_outputs):
        ...
        input_ = Input(shape=(height, width, channels))
        first_branch = branch(Lambda(lambda x: x[:, :, :, :3])(input_))
        second_branch = branch(Lambda(lambda x: x[:, :, :, 3:])(input_))
        if NORMALIZATION_ON:
    ...
    def build_pixel_comparison_network(input_shape):
        channels, height, width = input_shape
        input_ = Input(shape=(height, width, channels))
        first = Flatten()(Lambda(lambda x: x[:, :, :, :1])(input_))
        second = Flatten()(Lambda(lambda x: x[:, :, :, 1:])(input_))
